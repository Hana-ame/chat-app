...
func (d *DB) ListPublicChats(ctx context.Context, page, limit int) ([]models.Chat, error) {
	...
	rows, err := d.QueryContext(ctx,
		`SELECT c.id, c.type, c.name, c.icon_color, c.avatar_url, c.banner_url, c.banner_opacity, c.background_url, COALESCE(c.visibility,'private'), c.owner_id, c.created_at, c.last_message_at, c.pinned_message, c.pinned_updated_at,
	...
	for rows.Next() {
		...
		var created string
		var memberCount int
		if err := rows.Scan(&c.ID, &c.Type, &name, &c.IconColor, &c.AvatarURL, &c.BannerURL, &c.BannerOpacity, &c.BackgroundURL, &c.Visibility, &owner, &created, &lastMsg, &pinnedMsg, &pinnedUpdAt, &memberCount, &lastMsgContent); err != nil {
			return nil, err
		}
		c.Name = name.String
