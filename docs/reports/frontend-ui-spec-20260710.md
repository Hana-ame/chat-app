...
> 依赖：React 18、Lucide Icons、Tailwind CSS
...
- [MemberPanel 组件](#memberpanel-组件)
  ...
  - [DmSearchPanel.jsx — DM 搜索](#dmsearchpaneljsx--dm-搜索)
- [Modal 组件](#modal-组件)
...
- [样式系统](#样式系统)
  - [avatarColors — 头像颜色](#avatarcolors--头像颜色)
...
├── PublicChannelList.jsx   ← 公开频道列表（嵌入 MemberPanel）
├── DmSearchPanel.jsx       ← DM 搜索面板（⚠️ 未被引用）
├── EmptyState.jsx          ← 无聊天时的占位提示
├── WelcomeView.jsx         ← 首次访问欢迎提示
├── ScrollArea.jsx          ← 自定义滚动区域（无外部依赖）
└── renderContent.jsx       ← 消息内容解析器（markdown + emoji + urls）
```
...
**职责：**
1. 三栏布局：`sidebar (280px) | chat (flex-1) | member-panel (280px)`
...
| `useEffect([urlChatId])` | 生命周期 | URL → Store 同步：URL 中的 `:chatId` 变化时调用 `setActiveChat()` 切换活跃聊天 |
| `useEffect([activeChatId])` | 生命周期 | 消息加载 + 已读标记：切换聊天时加载历史消息，标记最后一条消息为已读 |
...
**布局结构：**
```html
<div class="h-screen flex flex-col bg-dark-900 text-gray-100">
  <nav> <!-- 顶部导航栏 --> </nav>
  <div class="flex-1 flex overflow-hidden">
    {isMobile ? (mobileView === 'list' ? <Sidebar/> : <ChatArea/>) : <Sidebar/> <ChatArea/> <MemberArea/>}
  </div>
</div>
```
...
**位置：** `client/src/components/ChatList.jsx`（229 行）
...
**使用场景：** 用户登录后看到所有已加入的聊天；输入搜索词过滤聊天名称；点击「+」按钮展开新建群组表单；右键聊天项弹出上下文菜单进行 Pin/查看信息/删除操作；点击底部用户头像打开设置弹窗。
...
| `handleCreate()` | 新建群组：校验名称非空 → 调用 `api.createChat()` → 关闭表单 → 选中新 chat |
| `handleDeleteChat(chatId)` | 删除聊天：弹出 confirm 确认 → 调用 `api.deleteChat()` → 关闭菜单 |
| `handleTogglePin(chatId)` | 切换置顶：调用 `api.togglePin()` → 关闭菜单。Store 的 `onChatUpdate` 自动处理排序 |
| `searchPublic(q)` | 搜索公开频道：调用 `api.listPublicChats()` 获取全部公开 chat → 按名称/ID 过滤 → 设置结果 |
| `handleJoinPublic(chatId)` | 加入公开频道：调用 `api.joinChat()` → 刷新聊天列表 → 选中新 chat |
| `joinChatByID(chatId)` | 通过 ID 加入聊天：调用 `api.joinChat()` → 清空搜索 → 刷新聊天列表 |
...
**位置：** `client/src/components/ChatListItem.jsx`（55 行）
...
| `onContextMenu` | ({chatId, x, y}) => void | 右键菜单回调（传递坐标） |

**函数：**

| 函数 | 职责 |
|------|------|
| `timeAgo(t)` | 时间格式化：刚刚（<1分钟）、Xm（<1小时）、Xh（<24小时）、月/日（其他） |
...
**位置：** `client/src/components/CreateGroupForm.jsx`
...
**位置：** `client/src/components/PublicChannelList.jsx`

**功能概述：** PublicChannelList 展示所有公开频道的列表，已加入的显示 ✅ 标记，未加入的显示「Join」按钮。嵌入 ChatList 和 MemberPanel 中使用。

**使用场景：** 搜索公开频道后显示结果列表；MemberPanel 底部显示所有公开频道供快速加入。
...
**⚠️ 注意：** 中英文混用（"Public Channels" + "加入成功" + "暂无公开频道"）。

---

### `EmptyState.jsx` — 空状态占位

**位置：** `client/src/components/EmptyState.jsx`

**功能概述：** 纯展示组件，无状态。在聊天列表为空时显示提示文案和创建按钮。
...
**位置：** `client/src/components/ChatView.jsx`（206 行）
...
| 函数 | 职责 |
|------|------|
| `getDMName(chat, currentUserId)` | 工具函数：DM 聊天取对方用户名，群组聊天取 chat.name |
...
| `handleSaveNotice()` | 保存 Notice：校验非空 → 调用 `setPinnedMessage()` → 退出编辑模式 |
| `handleClearNotice()` | 清除 Notice：调用 `clearPinnedMessage()` → 清空输入 → 退出编辑模式 |
...
**位置：** `client/src/components/MessageItem.jsx`（153 行）

**功能概述：** MessageItem 渲染单条消息的完整内容：头像、用户名、时间戳、消息气泡（支持 markdown）、附件、Reaction 列表、编辑/删除操作、流式输出指示器。点击头像或用户名可弹出 UserProfileModal。每个消息对应一个实例。
...
| 函数 | 职责 |
|------|------|
| `timeFormat(t)` | 工具函数：将 ISO 时间戳格式化为 `HH:mm` |
...
**位置：** `client/src/components/Composer.jsx`（100 行）

**功能概述：** Composer 是消息输入区域，包含多行文本框、附件上传、发送按钮、AI 快捷发送按钮。支持 Enter 发送、Shift+Enter 换行、自动高度调整、打字状态通知。

**使用场景：** 用户在 ChatView 底部输入消息；按 Enter 发送，Shift+Enter 换行；点击 📎 上传文件；点击 🤖 快捷发送消息给 AI（Mock 模式）；输入时自动通知其他人「正在输入」。
...
| `handleTyping()` | 通知 Store 发送 typing 事件 → 防抖清除旧 timer → 设新 timer（2 秒无输入后停止） |
| `handleSend()` | 发送消息：校验内容或附件非空 → 调用 `sendMessage()` → 清空输入和附件 |
| `handleKey(e)` | 键盘事件：Enter（非 Shift）→ 阻止默认 + 发送；其他键 → 触发 typing 通知 |
| `handleFile(e)` | 文件上传：遍历选中文件 → 逐个调用 `api.upload()` → 追加到 attachments → 清空 input |
...
- `#avatar-file-input` DOM 耦合（通过 `document.getElementById` 访问）。

---

### `WelcomeView.jsx` — 欢迎视图

**位置：** `client/src/components/WelcomeView.jsx`

**功能概述：** 纯展示组件，无状态。在未选择任何聊天时显示欢迎信息和创建按钮。
...
**位置：** `client/src/components/MemberPanel.jsx`（90 行）
...
### `DmSearchPanel.jsx` — DM 搜索

**位置：** `client/src/components/DmSearchPanel.jsx`

**功能概述：** DM 搜索面板，搜索用户并创建 DM 聊天。

**⚠️ 未被引用：** 任何组件都没有 import 此文件。属于死代码。
...
**位置：** `client/src/components/ChatInfoModal.jsx`（92 行）
...
**位置：** `client/src/components/SettingsModal.jsx`（45 行）
...
**位置：** `client/src/components/UserProfileModal.jsx`
...
**位置：** `client/src/components/ScrollArea.jsx`

**功能概述：** ScrollArea 封装一个带自定义滚动条样式的滚动容器，支持固定高度或 flex 填充。
...
**位置：** `client/src/components/renderContent.jsx`

**功能概述：** renderContent 将纯文本消息解析为 React 元素，支持 markdown 粗体/斜体/代码、URL 自动链接化、emoji 替换（`:emoji_name:` → emoji 字符）、`@mention` 解析（通过 userMap 映射用户名）。
...
文本 → 按行分割 → 逐行解析：
  ...
  - 代码：`code` → <code>
  - 链接：http(s)://... → <a>
  - emoji：:emoji_name: → emoji 字符
  - @mention：@username → <span class="mention">
```
...
**位置：** `client/src/components/ChatListItem.jsx`
...
**用途：** ChatListItem 和 ChatInfoModal 的头像背景色。

---

### Tailwind 工具类

**全局颜色：** `dark-900`（背景）、`dark-800`（侧边栏）、`dark-700`（分割线）、`dark-600`（hover）。

**常用模式：**
- `bg-indigo-500/10` — 未读 badge 背景
- `text-indigo-400` — 活跃状态文字
- `hover:bg-dark-600` — 交互反馈
- `border-b border-dark-700` — 分割线
...
| # | 问题 | 位置 | 严重性 |
|---|------|------|--------|
| 1 | `DmSearchPanel.jsx` 未被引用 | DmSearchPanel | 低 |
| 2 | `UserProfileModal.jsx` 未被正式引用 | UserProfileModal | 低 |
| 3 | 无虚拟列表，大量消息时性能差 | MessageItem, ChatListItem | 中 |
| 4 | `alert()`/`confirm()` 阻塞 UI | MessageItem, ChatInfoModal | 中 |
| 5 | `#avatar-color-input` / `#file-input` DOM 耦合 | SettingsModal, Composer | 低 |
| 6 | `renderContent` 不支持多行代码块 | renderContent | 中 |
