...
| **SQL** | `SELECT ... pinned_message, pinned_updated_at, member_count FROM chats WHERE id = ?` |
| **填充** | 填充 `LastMessage` 及其 `ID` |
| **使用场景** | 查询单个聊天详情时调用 |

```go
func (d *DB) GetChat(ctx context.Context, id string) (*models.Chat, error) {
    // SELECT id, type, name, icon_color, visibility, owner_id, created_at, last_message_at, last_message_id, pinned_message, pinned_updated_at, member_count FROM chats WHERE id = ?
...
| **SQL** | `SELECT c.id, c.type, c.name, c.icon_color, c.visibility, c.owner_id, c.created_at, c.last_message_at, c.last_message_id, cm.last_read_message_id, c.pinned_message, c.pinned_updated_at, c.member_count, cm.pinned_last_read_at FROM chat_members cm JOIN chats c ...` |
...
func (d *DB) ListPublicChats(ctx context.Context) ([]models.Chat, error) {
    rows, err := d.QueryContext(ctx,
        `SELECT id, type, name, icon_color, COALESCE(visibility,'private'),
                owner_id, created_at, last_message_at, pinned_message, pinned_updated_at,
...
#### `(d *DB) UpdatePinnedLastReadAt(ctx, chatID, userID string) error`

| 项 | 内容 |
|---|------|
| **操作** | **U** — 更新用户对该 chat 置顶消息的最后读取时间 |
| **目标 Model** | `chat_members.pinned_last_read_at` |
| **SQL** | `UPDATE chat_members SET pinned_last_read_at = ? WHERE chat_id = ? AND user_id = ?` |
| **时间格式** | `time.RFC3339Nano` |
| **使用场景** | 用户查看/确认置顶消息时调用（标记已读置顶） |
...
| `chats.go` | 15 | `RefreshToken` + `Chat` + `chat_members` | 含事务、含锁、含成员数缓存 |
...
CRUD 分布:
  ...
  Update: 9  (User ×3, Chat ×3, Message, chat_member, pinned_last_read_at)
...
| `db_test.go`（聊天 CRUD） | ~100 | CreateChat（group/dm）、FindDMBetween、ListUserChats、AddRemoveMember、DeleteChat、RenameChat、Set/ClearPinnedMessage、UpdatePinnedLastReadAt |
...
- 无 `ListPublicChats` / `JoinChatByID` 测试
- 无大数据量下的分页边界测试
