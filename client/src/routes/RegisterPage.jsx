...
export default function RegisterPage() {
  ...
  const { register, debugMode, mockLogin, loading, error } = useAuthStore();
  ...
  return (
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center', background:'var(--bg-tertiary)' }}>
      <form className="form-box" onSubmit={handle}>
        ...
        {debugMode && <button type="button" className="btn" style={{width:'100%',marginTop:8,background:'var(--danger)',color:'#fff'}} onClick={quickEnter}>
          ⚡ Debug Quick Enter (mock)
        </button>}
